...
//    adoption 게시판 리스트, detail 화면 기능 구현 완료 / admin 회원조회, 동물조회, 입양아이 게시판 기능 구현 완료
//    못 끝낸 기능: 우리아이자랑 게시판, 상품 게시판
...
//            // // 탭별 페이지 번호를 각각 분리해서 받음 - 안그러면 계속 회원관리 페이지 1로 넘어감
...
//        final int uPageSize  = 8; // 한 페이지에 보여줄 회원 수
...
//                case "userId":
//                    // 이메일(userEmail)으로 조회
//                    fullUserList = userDao.adminsearchPEmail(query);   // 이메일로 검색 매퍼
//                    break;
//                case "userName":
//                    // 이름(userName)으로 조회
//                    fullUserList = userDao.adminsearchPName(query);    // 이름으로 검색 매퍼
//                    break;
//                case "num":
//                    // seqno(userSeqno)로 조회
//                    try {
//                        int seq = Integer.parseInt(query);
...
//
//        // 페이징 계산
//        int uTotalCount = fullUserList.size(); // 전체 건수
...
//        int uOffset    = (userPage - 1) * uPageSize; // offset: (현재페이지 - 1) * 페이지크기
...
//        model.addAttribute("searchType",      searchType); // 선택된 검색 기준
//        model.addAttribute("query",           query); // 입력된 검색어
//
//
...
//            switch (searchType) {
//                case "breed":
//                    fullList = animalDao.searchByBreed(query);
//                    break;
//                case "region":
//                    fullList = animalDao.adminsearchAPlace(query);
//                    break;
//                case "num":
//                    try {
//                        int seq = Integer.parseInt(query);
...
//                case "breed":
//                    // 품종(aniBreed)으로 검색
//                    fullAdpList = animalDao.searchByBreed(query);
//                    break;
//                case "num":
//                    // seqno(aniSeqno)로 검색
//                    try {
...
//        model.addAttribute("postCurrentPage", 1);
//        model.addAttribute("postTotalPages", 1);
//        model.addAttribute("postStartPage", 1);
//        model.addAttribute("postEndPage", 1);
//        model.addAttribute("postSearchType", "writer+title");
//        model.addAttribute("postQuery", "");
