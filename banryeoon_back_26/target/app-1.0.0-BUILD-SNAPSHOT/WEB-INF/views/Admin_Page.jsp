    ...
    <table>
      <thead>
        <tr>
          ...
          <th class="s_regDate">가입일</th>
          <th class="s_yesno">입양여부</th>
...
<%--                <td class="s_regDate">${user.userRegDate}</td>--%>
                <td class="s_regDate">${user.userPhone}</td>
                <!-- 입양여부 -->
<%--                <td class="s_yesno">${user.adoptionYN}</td>--%>
                <td class="s_regDate">${user.userBirth}</td>
                <!-- 삭제 버튼 -->
...
<%--                          <a href="?page=${startPage - 1}" class="prev">« 이전</a>--%>
                          <a href="?userPage=${userStartPage - 1}&aniPage=${aniCurrentPage}#tab1" class="prev">« 이전</a>
                      </c:when>
                      <c:otherwise>
...
<%--                              <a href="?page=${i}" class="active">${i}</a>--%>
                              <a href="?page=${i}#tab1" class="active">${i}</a>
                          </c:when>
                          <c:otherwise>
<%--                              <a href="?page=${i}">${i}</a>--%>
                              <a href="?userPage=${i}&aniPage=${aniCurrentPage}#tab1">${i}</a>
                          </c:otherwise>
                      </c:choose>
                  </li>
              </c:forEach>
...
<%--                          <a href="?page=${endPage + 1}" class="next">다음 »</a>--%>
                          <a href="?userPage=${userEndPage + 1}&aniPage=${aniCurrentPage}#tab1" class="next">다음 »</a>
                      </c:when>
                      <c:otherwise>
      ...
      <thead>
        ...
        <c:forEach var="animal" items="${animals}" varStatus="st">
            <tr class="boardBody">
                ...
                <!-- 입양여부 -->
                    <%--                <td class="s_yesno">${user.adoptionYN}</td>--%>
                <td class="s_yesno">${animal.aniGender}</td>
                <!-- 삭제 버튼 -->
                <td class="s_out s_out_c">
...
<%--                          <a href="?page=${startPage - 1}" class="prev">« 이전</a>--%>
                          <a href="?aniPage=${aniStartPage - 1}&userPage=${userCurrentPage}#tab2"
                             class="prev">« 이전</a>
                      </c:when>
                      <c:otherwise>
                          <span class="prev disabled">« 이전</span>
...
<%--                              <a href="?page=${i}" class="active">${i}</a>--%>
                              <a href="?page=${i}#tab2" class="active">${i}</a>
                          </c:when>
                          <c:otherwise>
<%--                              <a href="?page=${i}">${i}</a>--%>
                              <a href="?aniPage=${i}&userPage=${userCurrentPage}#tab2">${i}</a>
                          </c:otherwise>
                      </c:choose>
                  </li>
              </c:forEach>
              ...
              <li>
                  <c:choose>
                      <c:when test="${aniEndPage < aniCurrentPage}">
                          <a href="?aniPage=${aniEndPage + 1}&userPage=${userCurrentPage}#tab2"
                             class="next">다음 »</a>
                      </c:when>
                      <c:otherwise>
                          <span class="next disabled">다음 »</span>
        ...
        <div class="thumbnail_box">
            <ul class="thumbnail_ul">
                <c:forEach var="post" items="${posts}" varStatus="st" begin="0" end="7">
                    <li class="thumbnail_li">
                        <a href="/post/detail?seq=${post.postId}">
                            ...
                            <div class="post_info">
                                ...
                                <div class="post_view">
                                    <div class="post_view_txt">조회<span class="post_view_num">${post.views}</span></div>
                                </div>
                            </div>
                        </a>
                    </li>
                ...
                <li>
                    <c:choose>
                        <c:when test="${postStartPage > 1}">
                            <a href="?postSearchType=${postSearchType}
                          &postQuery=${postQuery}
                          &postPage=${postStartPage-1}
                          &userPage=${userCurrentPage}
                          &aniPage=${aniCurrentPage}
                          &adpPage=${adpCurrentPage}
                          &prodPage=${prodCurrentPage}
              ...
              <li>
                  <c:choose>
                      <c:when test="${adpStartPage > 1}">
                          <a href="?adpPage=${adpStartPage-1}&userPage=${userCurrentPage}&aniPage=${aniCurrentPage}#tab4"
                             class="prev">« 이전</a>
                      </c:when>
                      <c:otherwise>
                          <span class="prev disabled">« 이전</span>
                  ...
                  <li>
                      <c:choose>
                          <c:when test="${i == adpCurrentPage}">
                              <a href="?adpPage=${i}&userPage=${userCurrentPage}&aniPage=${aniCurrentPage}#tab4"
                                 class="active">${i}</a>
                          </c:when>
                          <c:otherwise>
                              <a href="?adpPage=${i}&userPage=${userCurrentPage}&aniPage=${aniCurrentPage}#tab4">${i}</a>
                          </c:otherwise>
                      </c:choose>
                  </li>
              </c:forEach>
              ...
              <li>
                  <c:choose>
                      <c:when test="${adpEndPage < adpTotalPages}">
                          <a href="?adpPage=${adpEndPage+1}&userPage=${userCurrentPage}&aniPage=${aniCurrentPage}#tab4"
                             class="next">다음 »</a>
                      </c:when>
                      <c:otherwise>
                          <span class="next disabled">다음 »</span>
