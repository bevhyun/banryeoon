...
COMMIT;
SELECT * FROM animal WHERE aniHappenPlace LIKE CONCAT('%서울%');

        SELECT * FROM animal WHERE aniHappenPlace LIKE CONCAT('%서울%');
        SELECT aniBreed, aniGender, aniAge, aniWeight, aniImg1, aniImg2, aniHappenDt, aniSpecialMark FROM banryeoon_db.animal WHERE aniTypeCd = 100;
        SELECT * FROM animal WHERE aniBreed LIKE CONCAT('%믹%');
